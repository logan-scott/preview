...
char *pdf_render_inproc(const source_file *src) {
    ...
    sb_append(&s,
              ...
              /* pdf.js text layer: invisible but selectable text over the page */
              ...
              ".textLayer ::selection{background:rgba(64,147,248,.4)}"
    ...
    sb_append(
        ...
        "        var scale=Math.min(2,820/base.width);\n"
        ...
        "        wrap.appendChild(tl);\n"
        "        var rt=dpr!==1?[dpr,0,0,dpr,0,0]:null;\n"
        "        return page.render({canvasContext:c.getContext('2d'),"
        "viewport:vp,transform:rt}).promise.then(function(){\n"
        "          return page.getTextContent();}).then(function(tc){\n"
        "          return pdfjsLib.renderTextLayer({textContentSource:tc,"
        "container:tl,viewport:vp}).promise;}).catch(function(){});\n"
        "      });});})(p);}\n"
